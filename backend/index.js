...
// POST /api/natal/calculate — calculate natal chart + save to user
app.post("/api/natal/calculate", (req, res) => {
  try {
    ...
    console.log(`[Natal] Chart calculated for ${deviceId.substring(0, 12)}... → ☉${chart.sunSign} ☽${chart.moonSign} ASC:${chart.risingSign || "?"}`);
...
// POST /api/natal/save — save natal chart directly (sign-based, no calculation)
app.post("/api/natal/save", (req, res) => {
  try {
    ...
    updateSharedUser(deviceId, { natalChart: chart });
    console.log(`[Natal] Saved for ${deviceId.substring(0, 12)}... → ☉${chart.sunSign} ☽${chart.moonSign} ASC:${chart.risingSign || "?"}`);
...
// GET /api/natal/:deviceId — get stored natal chart
app.get("/api/natal/:deviceId", (req, res) => {
  try {
    const user = getSharedUser(req.params.deviceId);
    if (!user.natalChart) {
...
// ============================================
// NATAL CHART INTERPRETATION (One-time AI reading)
// ============================================
...
const MOCK_NATAL_CHART = {
  ...
  planets: [
    { name: "sun", symbol: "☉", sign: "libra", degree: 26, minute: 38, house: 5, retrograde: false, lng: 206.65 },
    { name: "moon", symbol: "☽", sign: "libra", degree: 7, minute: 36, house: 4, retrograde: false, lng: 187.60 },
    { name: "mercury", symbol: "☿", sign: "scorpio", degree: 18, minute: 32, house: 5, retrograde: false, lng: 228.54 },
    { name: "venus", symbol: "♀", sign: "libra", degree: 7, minute: 20, house: 4, retrograde: false, lng: 187.34 },
    { name: "mars", symbol: "♂", sign: "scorpio", degree: 18, minute: 50, house: 5, retrograde: false, lng: 228.84 },
    { name: "jupiter", symbol: "♃", sign: "cancer", degree: 24, minute: 18, house: 1, retrograde: false, lng: 114.31 },
    { name: "saturn", symbol: "♄", sign: "pisces", degree: 26, minute: 27, house: 10, retrograde: true, lng: 356.45 },
    { name: "uranus", symbol: "♅", sign: "gemini", degree: 0, minute: 42, house: 11, retrograde: true, lng: 60.71 },
    { name: "neptune", symbol: "♆", sign: "aries", degree: 0, minute: 3, house: 10, retrograde: true, lng: 0.06 },
    { name: "pluto", symbol: "♇", sign: "aquarius", degree: 1, minute: 22, house: 8, retrograde: false, lng: 301.37 },
    { name: "north_node", symbol: "☊", sign: "pisces", degree: 17, minute: 57, house: 10, retrograde: true, lng: 347.95 },
    { name: "south_node", symbol: "☋", sign: "virgo", degree: 17, minute: 57, house: 4, retrograde: false, lng: 167.95 },
    { name: "black_moon", symbol: "⚸", sign: "scorpio", degree: 23, minute: 3, house: 5, retrograde: false, lng: 233.06 },
    { name: "asc", symbol: "ASC", sign: "cancer", degree: 7, minute: 4, lng: 97.08 },
    { name: "mc", symbol: "MC", sign: "pisces", degree: 15, minute: 34, lng: 345.57 },
  ],
};

// ── Natal Chart Enrichment Helpers ──
  ...
  for (let i = 0; i < eligible.length; i++) {
    for (let j = i + 1; j < eligible.length; j++) {
      ...
      for (const asp of ASPECT_DEFS) {
        ...
        if (diff <= asp.orb) {
          aspects.push({
            planet1: a.name, symbol1: a.symbol,
            planet2: b.name, symbol2: b.symbol,
            aspect: asp.name, exactDeg: Math.round(dist * 10) / 10,
...
function findHouseClusters(planets) {
  ...
  return Object.entries(houses)
    ...
    .map(([h, ps]) => ({ house: parseInt(h), planets: ps, count: ps.length }));
}
...
function buildEnrichedNatalContext(planets, lang) {
  ...
  return {
    summary: `GEZEGEN KONUMLARI:\n${planetLines}\n\nÖNEMLİ AÇILAR (ASPECTS):\n${aspectLines || "Yok"}\n\nRETROGRADE GEZEGENLER:\n${retroLines || "Yok"}\n\nEV YOĞUNLUKLARI:\n${clusterLines || "Yok"}\n\nELEMENT DENGESİ: Ateş:${elements.fire} Toprak:${elements.earth} Hava:${elements.air} Su:${elements.water} | Baskın: ${elements.dominant}\nMODALİTE: Kardinal:${modality.cardinal} Sabit:${modality.fixed} Değişken:${modality.mutable} | Baskın: ${modality.dominant}`,
    aspects: topAspects,
    retrogrades,
    houseClusters,
    elements,
...
function loadNatalReadingCache(lang) {
  ...
  } catch (e) { console.warn(`[Natal] Cache read error (${lang}):`, e.message); }
  return { readings: {} };
}
...
function buildNatalSummary(planets, lang) {
  const names = ZODIAC_NAMES_NATAL[lang] || ZODIAC_NAMES_NATAL.tr;
...
// POST /api/natal/interpret — generate AI interpretation (one-time, 50 Gemstone)
app.post("/api/natal/interpret", async (req, res) => {
  try {
    ...
    // Check if already interpreted (one-time only)
    ...
    // Check gemstone balance
    if (user.gemstoneBalance < NATAL_INTERPRET_COST) {
      return res.status(402).json({
        success: false,
        error: "INSUFFICIENT_GEMSTONES",
    ...
    // Use user's natal chart or mock data
    ...
    // Debit gemstones
    updateSharedUser(deviceId, { gemstoneBalance: user.gemstoneBalance - NATAL_INTERPRET_COST });
...
// GET /api/natal/interpret/:deviceId — get cached interpretation
app.get("/api/natal/interpret/:deviceId", (req, res) => {
  try {
...
// GET /api/natal/mock — return mock natal chart data (for settings display)
app.get("/api/natal/mock", (req, res) => {
  res.json({ success: true, data: MOCK_NATAL_CHART });
});

// DELETE /api/natal/interpret/:deviceId — delete cached interpretation (allows re-generation)
app.delete("/api/natal/interpret/:deviceId", (req, res) => {
  try {
    ...
    res.json({ success: true, deleted, message: deleted ? "Natal yorum silindi. Tekrar yorumlatabilirsiniz." : "Silinecek yorum bulunamadı." });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
});
